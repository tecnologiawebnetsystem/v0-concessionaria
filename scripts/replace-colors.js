...
// Mapeamento de substituições (mais específico primeiro)
const replacements = [
  // Gradientes blue→cyan para red/gray
  ["from-blue-950 via-blue-900 to-indigo-900", "from-gray-950 via-gray-900 to-red-950"],
  ["from-blue-950 via-blue-900", "from-gray-950 via-gray-900"],
  ...
  ["nacionalveiculos.com.br", "gtveiculos.com.br"],
  ["nacional.com.br", "gtveiculos.com.br"],
]
...
console.log("[v0] rootDir:", rootDir)
try {
  const top = fs.readdirSync(rootDir)
  console.log("[v0] top-level entries:", top.slice(0,10).join(", "))
} catch(e) {
  console.log("[v0] erro ao ler rootDir:", e.message)
}
let totalFiles = 0
...
function shouldSkip(filePath) {
  return skipDirs.some(d => filePath.includes("/" + d + "/"))
}
...
walkDir(rootDir, (filePath) => {
  ...
  if (content !== original) {
    ...
    console.log(`[v0] Atualizado: ${rel} (${count} subs)`)
  }
})

console.log(`\n[v0] TOTAL: ${totalFiles} arquivo(s), ${totalReplacements} substituição(ões)`)
