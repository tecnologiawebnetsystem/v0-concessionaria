...
import { Heart, Car, ArrowRight, Trash2, Calendar, Gauge, Fuel, Loader2 } from "lucide-react"
...
export default function CustomerFavoritesPage() {
  const { data, isLoading } = useSWR<{ favorites: FavoriteVehicle[] }>("/api/favorites", fetcher)
  ...
  async function clearAll() {
    await Promise.all(favorites.map((v) => removeFavorite(v.id)))
  }
        ...
        <>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {favorites.map((vehicle) => (
              <Card key={vehicle.favorite_id} className="overflow-hidden group bg-slate-800/50 border-slate-700">
                <div className="relative aspect-video">
                  <Image
                    src={vehicle.main_image || "https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=600&q=80"}
                    alt={vehicle.name}
                    fill
